...
2. **雷達回波大範圍降雨**：5km 半徑內，回波像素點數 >= 20（`echo_pixel_threshold_cover`）。
3. **雷達回波強烈微爆流**：5km 半徑內，即使範圍極小，只要「最大 dBZ」>= 45（`max_dbz_threshold_cover`，通常對應紅色暴雨）。

### 4. 停雨（解除加蓋）判定
目前處於「加蓋」狀態時，必須 **同時滿足** 以下三個條件，才會解除加蓋並發送 `uncover` 通知：
1. **雨量計無雨**：安南測站雨量為 0。
2. **雷達回波無雲層接近**：5km 半徑內，回波像素點數 < 10（`echo_pixel_threshold_uncover`），且最大 dBZ < 30（`max_dbz_threshold_uncover`）。
